...
class Vite {
   ...
   * @throws Exception
   */
  public static function asset($asset, bool $enqueueCss = true): string {
    if (static::$isHot) {
  ...
  /**
   * Enqueue the sub assets for a given asset.
   *
   * @param $asset
   *
   * @return void
   */
  private static function enqueueSubModules($asset): void {
    // If we're not hot, enqueue asset css dependencies
    if (array_key_exists('css', static::$manifest[$asset])) {
      foreach (static::$manifest[$asset]['css'] as $cssAsset) {
        $enqueueStyleArgs = [
          basename($cssAsset, ".css"),
          implode('/', [get_stylesheet_directory_uri(), static::$buildPath, $cssAsset]),
          [],
          null,
          'screen'
        ];

        wp_enqueue_style(...$enqueueStyleArgs);
      }
    }
  }
